...
Deciding things together is a feature, not an afterthought. Bonfire for communities ships with decision-making tools that range from quick polls to structured consensus processes, so groups can use the lightest process that fits the decision:
...
And you don't have to host it alone: host it yourself, or lean on the [cooperative hosting network](/hosting/) we're building with allied co-ops (more on that in the [FAQ](#faq)).
